    ...
    public static boolean isCreatureType(Entity entity, EnumCreatureType type) {
        ...
        if (spawnTypeOpt == null) {
            ...
            if (entity.getClass()
                ...
                LOGGER.debug(
                    "Creature " + entity.getClass()
                        .getName()
                        + " has no type according to vanilla logic, leaving type as default: "
                        + vanillaTypes);
                spawnType = null;
            } else {
                spawnType = computeSpawnType(entity);
                if (spawnType == null) {
                    // e.g. Reika.ChromatiCraft.Entity.EntityBallLightning has no type, and uses custom logic for
                    // despawning
                    LOGGER.debug(
                        "Creature " + entity.getClass()
                            .getName()
                            + " is not in the biome spawn registry, leaving type as default: "
                            + vanillaTypes);
            ...
            if (vanillaTypes.size() > 1 || (vanillaTypes.size() == 1 && vanillaTypes.get(0) != spawnType)) {
                LOGGER.debug(
                    "Changed creature type of " + entity.getClass()
                        .getName() + " from " + vanillaTypes + " to " + spawnType);
            }
        }
            ...
            if (candidateCount != 1) {
                ...
                LOGGER.debug(
                    "Entity " + entity.getClass()
                        .getName() + " has multiple spawn types registered: " + types + ". Choosing " + type);
            }

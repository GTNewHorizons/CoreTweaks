...
public class ConfigMigrator {
    ...
    public void migrate_0_2_to_0_3() {
        ...
        migrateStringList(
            ...
            "transformersToCache",
            (x) -> transformersToCache = new StringList(x));
    ...
    public void migrate_0_3_0_to_0_3_1(boolean newer) {
        ...
        if (newer) {
            if (((Setting) Config.extendSprintTimeLimit.getValue()) == Setting.FALSE) {
                warn(
                    "0.3.1",
                    "The default value of extend_sprint_time_limit has changed to TRUE, but you have it set to the old default value of FALSE. You might wish to enable it.");
            }
            if (((Setting) Config.fastDefaultResourcePack.getValue()) == Setting.FALSE) {
                warn(
                    "0.3.1",
                    "The default value of fast_folder_resource_pack has changed to TRUE, but you have it set to the old default value of FALSE. You might wish to enable it.");
            }
        }
    ...
    private void migrateRenamedFeatureSetting(String category, FeatureSetting setting) {
        try {
            if (config.hasCategory(category)) {
                ...
                if (cat != null) {
                    setting.setValue(
                        FeatureSetting.Setting.valueOf(
                            cat.get("_enabled")
                                .getString()
                                .toUpperCase()));
                    cat.remove("_enabled");
    ...
    private void deleteStringIfDefault(String cat, String name, String def) {
        if (config.hasCategory(cat)) {
            ...
            if (category.containsKey(name)) {
                Property prop = category.get(name);
                if (prop.getType() == Type.STRING && prop.getString()
                    .equals(def)) {
                    category.remove(name);
                }
            }
    ...
    private void migrateEnum(String cat, String name, Consumer<Enum<?>> consumer, Class<? extends Enum> enumClass) {
        if (config.hasCategory(cat)) {
            ...
            if (category.containsKey(name)) {
                ...
                if (prop.getType() == Type.STRING) {
                    consumer.accept(
                        (Enum<?>) EnumUtils.getEnumMap(enumClass)
                            .get(
                                prop.getString()
                                    .toUpperCase()));
                    category.remove(name);

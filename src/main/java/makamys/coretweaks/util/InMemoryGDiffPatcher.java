...
public class InMemoryGDiffPatcher {
    ...
    public static void patch(byte[] source, byte[] patch, byte[] dest) {
        if (!(patch[0] == (byte) 0xD1 && patch[1] == (byte) 0xFF
            && patch[2] == (byte) 0xD1
            && patch[3] == (byte) 0xFF)) {
    ...
    private static int readValue(byte[] array, int offset, int type) {
        switch (type) {
            ...
            case INT:
                return (Byte.toUnsignedInt(array[offset]) << 24) | (Byte.toUnsignedInt(array[offset + 1]) << 16)
                    | (Byte.toUnsignedInt(array[offset + 2]) << 8)
                    | (Byte.toUnsignedInt(array[offset + 3]) << 0);
            default:

...
public class AnnotationBasedConfigHelper {
    ...
    private void setConfigClassField(Field field, Object newValue, Configuration config) {
        try {
            if (IWrappedEnum.class.isAssignableFrom(field.getType())) {
                ...
                if (instance == null) {
                    instance = (IWrappedEnum) field.getType()
                        .newInstance();
    ...
    private boolean iterateOverConfigAnd(Configuration config, TriConsumer<Field, Object, Configuration> callback) {
        ...
        for (Field field : theConfigClass.getFields()) {
            ...
            if (configBoolean == null && configInt == null
                && configFloat == null
                && configEnum == null
                && configWrappedEnum == null
                && configStringList == null
                && configString == null) continue;
            ...
            if (configBoolean != null) {
                newValue = config.getBoolean(
                    field.getName(),
                    snakeifyCategory(configBoolean.cat()),
                    configBoolean.def(),
                    configBoolean.com());
            } else if (configInt != null) {
                newValue = config.getInt(
                    field.getName(),
                    snakeifyCategory(configInt.cat()),
                    configInt.def(),
                    configInt.min(),
                    configInt.max(),
                    configInt.com());
            } else if (configFloat != null) {
                newValue = config.getFloat(
                    field.getName(),
                    snakeifyCategory(configFloat.cat()),
                    configFloat.def(),
                    configFloat.min(),
                    configFloat.max(),
                    configFloat.com());
                ...
                WrappedEnum loadableAnn = ((WrappedEnum) field.getType()
                    .getAnnotation(WrappedEnum.class));
                ...
                boolean lowerCase = annDef.codePoints()
                    .anyMatch(cp -> Character.isLowerCase(cp));

                Class<? extends Enum> configClass = (Class<? extends Enum>) (configEnum != null ? field.getType()
                    : loadableAnn.enumClass());
                Map<String, ? extends Enum> enumMap = EnumUtils.getEnumMap(configClass);
                String[] valuesStrUpper = (String[]) enumMap.keySet()
                    .stream()
                    .toArray(String[]::new);
                String[] valuesStr = Arrays.stream(valuesStrUpper)
                ...
                String newValueStr = config.getString(
                    propName,
                    annCat,
                    lowerCase ? annDef.toLowerCase() : annDef.toUpperCase(),
                    annCom,
                    valuesStr);
                if (oldVal != null) {
                    newValueStr = oldVal;
                ...
                String[] strings = config
                    .getStringList(field.getName(), snakeifyCategory(configStringList.cat()), defaultVal, comment);
                if (strings.length > 0 && strings[0].equals(":resetOnLoad")) {
                    if (!Arrays.equals(strings, defaultVal)) {
                        // reset
                        config.getCategory(snakeifyCategory(configStringList.cat()))
                            .remove(field.getName());
                        config.getStringList(
                            field.getName(),
                            snakeifyCategory(configStringList.cat()),
                            defaultVal,
                            comment);
                    }
                    strings = defaultVal;
                ...
                newValue = config.getString(
                    field.getName(),
                    snakeifyCategory(configString.cat()),
                    configString.def(),
                    configString.com());
            }
    ...
    private static String camelCaseToSnakeCase(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (i > 0 && i < s.length()
                ...
                spaceField[i] = true;
            } else if (i > 2 && Character.isLetter(s.charAt(i - 2))
                && Character.isUpperCase(s.charAt(i - 2))
                && Character.isLetter(s.charAt(i - 1))
                && Character.isUpperCase(s.charAt(i - 1))
                && Character.isLetter(s.charAt(i))
                && !Character.isUpperCase(s.charAt(i))) {
    ...
    private static String snakeifyCategory(String annCat) {
        return String.join(
            ".",
            Arrays.stream(annCat.split("\\."))
                .map(s -> camelCaseToSnakeCase(s))
                .toArray(String[]::new));
    }
                ...
                if (!fieldValue.equals(newValue)) {
                    for (String catName : config.getCategoryNames()) {
                        ...
                        if (fieldValue instanceof IWrappedEnum) {
                            ...
                            WrappedEnum wrappedEnumAnn = wrappedEnum.getClass()
                                .getAnnotation(WrappedEnum.class);
                            if (!wrappedEnumAnn.categoryProperty()
                                .isEmpty()) {
                        ...
                        if (config.getCategoryNames()
                            .contains(snakeifiedCatName)) {
        ...
        switch (prop.getType()) {
            ...
            case STRING:
                if (newValue instanceof String[]) {
                    ...
                    if (newValue instanceof Enum && prop.getDefault()
                        .toLowerCase()
                        .equals(prop.getDefault())) {
                        val = val.toLowerCase();
                    }
                    prop.set(val);
                }

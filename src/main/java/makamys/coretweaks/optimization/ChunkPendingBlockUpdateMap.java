...
public class ChunkPendingBlockUpdateMap {
    ...
    public static final boolean DEBUG = Boolean
        .parseBoolean(System.getProperty("coretweaks.optimizeGetPendingBlockUpdates.debug", "false"));
    public static final boolean LENIENT_MODE = Boolean
        .parseBoolean(System.getProperty("coretweaks.optimizeGetPendingBlockUpdates.lenient", "false"));
    ...
    public static void onTick(IPendingBlockUpdatesWorldServer ws) {
        if (DEBUG) {
            Map<Long, Set<NextTickListEntry>> map = ws.crtw$getChunkPendingUpdatesMap();
            Iterator<Entry<Long, Set<NextTickListEntry>>> it = map.entrySet()
                .iterator();
            String debug = "";
            while (it.hasNext()) {

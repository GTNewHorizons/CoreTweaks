...
public class ClientChunkMap extends LongHashMap {
    ...
    private int chunkXinternal2world(int x) {
        return (Math.floorDiv(offX, getDiameter()) + (x < (Math.floorMod(offX, getDiameter())) ? 1 : 0)) * getDiameter()
            + x;
    }

    private int chunkZinternal2world(int z) {
        return (Math.floorDiv(offZ, getDiameter()) + (z < (Math.floorMod(offZ, getDiameter())) ? 1 : 0)) * getDiameter()
            + z;
    }

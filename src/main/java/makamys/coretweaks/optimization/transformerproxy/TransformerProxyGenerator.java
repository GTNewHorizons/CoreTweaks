...
/**
 * <p>
 * Generates a proxy class that wraps a class transformer without changing its functionality (a subclass of either
 * {@link TransformerProxy} or {@link NameTransformerProxy}).
 * </p>
 * <p>
 * The generated class has the same full name as the original class, only with the <code>$$CoreTweaksProxy</code> suffix
 * added. This allows
 * seamless interoperation with mods like Mixin that do class name checks.
 * </p>
 */
public class TransformerProxyGenerator implements Opcodes {

    private static final boolean DEBUG = Boolean
        .parseBoolean(System.getProperty("coretweaks.debugTransformerProxyGenerator", "false"));
    ...
    private static byte[] generateBytecode(String parentName, String transName) {
        ...
        cw.visit(
            ...
            transName.replace('.', '/') + "$$CoreTweaksProxy",
            null,
            parentName.replace('.', '/'),
            null);

        {
            ...
            mv.visitMethodInsn(
                ...
                parentName.replace('.', '/'),
                "<init>",
                "(Lnet/minecraft/launchwrapper/IClassTransformer;)V",
                false);
            mv.visitInsn(RETURN);
            mv.visitMaxs(2, 2);
            mv.visitEnd();

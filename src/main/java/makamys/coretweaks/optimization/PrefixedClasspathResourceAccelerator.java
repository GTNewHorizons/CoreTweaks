...
/**
 * <p>
 * If you frequently call {@link Class#getResourceAsStream(String)} with the same directory prefix, using this class
 * will speed up the operation.
 * <p>
 * It will only search for the resource in the jars that contain the directory.
 * <p>
 * However, if the classpath is modified after this class is instantiated, incorrect results may be returned. I sure
    ...
    public InputStream getResourceAsStream(String path) {
        ...
        if (DEBUG) {
            if ((vanillaResult == null) != (result == null)) {
                LOGGER.error(
                    "Mismatch detected in DefaultResourcePack optimization! (path=" + path
                        + ", vanillaResult="
                        + vanillaResult
                        + ", result="
                        + result
                        + ") Please report the issue and disable the option (`fast_default_resource_pack`) for now.");
            }
        }
        return result;
    }

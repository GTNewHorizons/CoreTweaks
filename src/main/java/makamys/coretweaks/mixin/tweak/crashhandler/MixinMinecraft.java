                ...
                while (this.running) {
                    if (!this.hasCrashed || this.crashReporter == null) {
                        try {
                            ...
                            CoreTweaks.LOGGER
                                .error("#@!@# Game crashed! CoreTweaks will attempt to keep the game running.");
                            t.printStackTrace();

                            if (!(t instanceof OutOfMemoryError)) {
                                // Create crash report, mirroring logic in Minecraft#run
                                // Note: addGraphicsAndWorldToCrashReport has to be called inside the same method as
                                // runGameLoop,
                                ...
                                if (crashReporter != null) {
                                    ...
                                    CrashReport cr = Minecraft.getMinecraft()
                                        .addGraphicsAndWorldToCrashReport(new CrashReport("Unexpected error", t));
                            ...
                            if (!(screen instanceof GuiMemoryErrorScreen || screen instanceof GuiFatalErrorScreen)) {
                                ...
                                this.displayGuiScreen(
                                    t instanceof OutOfMemoryError ? new GuiMemoryErrorScreen()
                                        : new GuiFatalErrorScreen(t));
                                System.gc();
                            } else {
                                CoreTweaks.LOGGER
                                    .warn("Got a crash on a crash error screen, quitting game to avoid infinite loop.");
                                this.running = false;
                            }
                        }
